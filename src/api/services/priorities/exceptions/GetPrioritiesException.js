...
export const GetPrioritiesException = (error) => {
    let response = { success: false, message: "", data: null }
    let status = error.response ? error.response.status : null;
    switch (status) {
        case 400:
            return {...response, message: "Ocurrio un error al consultar el listado de prioridades validas, porfavor intentalo mas tarde"}
        case 401:
